...
export const PRESETS: Record<string, Partial<Record<Axis, AxisPresetValue[]>>> = {
  driving: {
    region: [
...
export function applyPreset(domain: string, key: string): Record<string, number> {
  const presetKey = VERTICAL_TO_PRESET[key] ?? key;
  const preset = PRESETS[presetKey];
  if (!preset) return {};
  const summary: Record<string, number> = {};
  for (const [axis, values] of Object.entries(preset) as [Axis, AxisPresetValue[]][]) {
    if (!values || values.length === 0) continue;
    bulkReplaceAxis({ tenant: domain, axis, values });
